...
_IMAGE_CAPTION_PROMPT = (
    ...
    "После цитат добавь одно короткое предложение с назначением "
    "изображения.\n"
    "\n"
    "Правила выбора типа изображения:\n"
    "— «Блок-схема» пиши ТОЛЬКО если на самом изображении есть "
    "буквальная подпись «блок-схема» или «схема процесса», либо "
    "изображение состоит из очевидных прямоугольных блоков, "
    "соединённых стрелками с текстом внутри блоков. Скриншоты "
    "интерфейсов, окна программ и таблицы блок-схемой НЕ являются, "
    "даже если у них есть стрелки-указатели.\n"
    "— Если это окно 1С, «Документооборот», «Мой кабинет», формы, "
    "меню, панели инструментов — пиши «скриншот интерфейса».\n"
    "— Если это плакат, инфографика, слайд презентации с крупным "
    "текстом и оранжево-синими блоками FAQ — пиши «инфографика».\n"
    "— Если это фотография человека, иконка, логотип — так и "
    "называй.\n"
    "\n"
...
def _attach_images_to_answer(
    ...
    # Разбиваем ответ на пункты. Поддерживаем оба формата:
    # 1) нумерованный (1. 2. 3. или 1) 2) 3))
    # 2) маркированный буллетами (• пункт)
    ...
    points = numbered_re.findall(answer)
    if not points:
        points = bullet_re.findall(answer)
    if not points:
...
async def ask(req: AskRequest, request: Request):
    ...
    try:
        ...
        # «поставщиковТермолэнд»), чанки этого файла идут в контекст
        # ПЕРВЫМИ. Вопрос «состав ТК термолэнд» должен опираться на СТО
        # Термолэнда, а не на почти дословный СТО другой организации:
        # маленькая модель сильнее всего цепляется за начало контекста.
        _q_long_words = re.findall(r"[а-яёa-z0-9]{5,}", req.question.lower())
        _ctx_ids = results["ids"][0]
        if _q_long_words and _ctx_ids:
            _aff_by_id = {}
            for _rid, _m in zip(results["ids"][0], results["metadatas"][0]):
                _fn_l = _m["filename"].lower()
                _aff_by_id[_rid] = sum(1 for w in _q_long_words if w in _fn_l)
            if max(_aff_by_id.values()) > 0:
                _orig_order = {rid: i for i, rid in enumerate(_ctx_ids)}
                _ctx_ids = sorted(
                    _ctx_ids,
                    key=lambda rid: (-_aff_by_id[rid], _orig_order[rid]),
                )
                log.info(
                    "[context] file affinity reorder: %s",
                    {
                        _m["filename"][:45]: _aff_by_id[_rid]
                        for _rid, _m in zip(
        ...
        all_docs, all_metas = _build_document_aware_context(
            _ctx_ids, records, budget=top_k * 2,
        )
        ...
        if is_broad:
            system_content = (
                ...
                "Пустая строка.\n"
                "Дальше маркированный список: каждый пункт начинается с символа "
                "«• » (буллет + пробел). Никакой нумерации, никаких «1.», «2)», "
                "«— ». Каждый пункт — 2–3 полных предложения, живой связный "
                "текст. Не обрывок, не рубленая фраза, не заголовок.\n"
                "\n"
                "ПРИМЕР 1 (про валюту):\n"
                ...
                "\n"
                "• Сотрудник создаёт заявку на отпуск в 1С минимум за две недели "
                "до желаемой даты. Указывает период, тип отпуска и остаток дней.\n"
                "• Заявка автоматически уходит на согласование руководителю "
                "подразделения. Статус можно отслеживать в карточке заявки.\n"
                "• После согласования отпуск попадает в общий график, а сотруднику "
                ...
                "\n"
                "ВАЖНО — если в вопросе названа конкретная организация, проект "
                "или система (например, «Термолэнд»): бери факты ТОЛЬКО из "
                "документов про эту организацию (первые CHUNK в контексте). "
                "В контексте могут быть похожие документы других организаций с "
                "другими должностями и правилами — их НЕ используй и не "
                "смешивай с данными нужной организации.\n"
                "\n"
                "Если в контексте нет сути вопроса — ответь ровно одной фразой: "
                "Информация в документах не найдена.\n"
                "\n"
                "ЧТО ТОЧНО НЕ ДЕЛАТЬ:\n"
                "— Не нумеруй пункты. Используй только «• » как маркер.\n"
            ...
            system_content = (
                ...
                "• Баланс отражается в личном кабинете на корпоративном портале "
                ...
                [
                    ...
                    },
                    {
                        ...
                        "content": (
                            ...
                            # Дубль требований формата в КОНЦЕ промта:
                            # модели (особенно qwen в режиме без
                            # размышлений) сильнее всего весят конец
                            # пользовательского сообщения и без напоминания
                            # отвечают одним куцым пунктом без введения.
                            "Формат ответа (обязательно): сначала 1–2 "
                            "вводных предложения о сути темы, затем пустая "
                            "строка, затем 3–6 пунктов, каждый начинается "
                            "с «• » и содержит 2–3 полных предложения. "
                            "Пиши подробно, раскрой все относящиеся к "
                            "вопросу детали из контекста."
                        ),
                    },
                ],
                max_tokens=2000,
